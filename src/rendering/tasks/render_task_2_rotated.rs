...
impl Application {
	pub fn render_task_2_rotated(&mut self, ctx: &egui::Context) {
		egui::CentralPanel::default().show(ctx, |ui| {
			...
			let plot = egui::plot::Plot::new("Rotated orbits of planets")
				.data_aspect(1.0)
			...
			let radius = self.data.task_2_rotated_data.plot_width * MARKERS_RADIUS_FRACTION;
			let mut marker_lines = Vec::new();
			let mut orbits = Vec::new();
			for (points, colour, index, name, add_marker) in &self.data.task_2_rotated_data.points {
			...
			self.data.task_2_rotated_data.plot_width = plot_width;
		});
	}
}
